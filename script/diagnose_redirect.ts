...
async function diagnose() {
    ...
    conn.on("ready", async () => {
        ...
        try {
            console.log("--- Checking Enabled Sites ---");
            await executeCommand(conn, "ls -l /etc/nginx/sites-enabled/");

            console.log("\n--- Checking default config (if exists) ---");
            await executeCommand(conn, "cat /etc/nginx/sites-enabled/default || echo 'No default file'");

            console.log("\n--- Checking muhammadsameer config ---");
            await executeCommand(conn, "cat /etc/nginx/sites-enabled/muhammadsameer");

            console.log("\n--- Checking pdf-converters config ---");
            await executeCommand(conn, "cat /etc/nginx/sites-enabled/pdf-converters.online");
